...
class ResNet(nn.Module):
  def __init__(self, block, layers, num_classes=1000):
    if cfg.add_filter_ratio >= 0.005: 
      self.inplanes = 64 + int(64*cfg.add_filter_ratio)
      cfg.add_filter_num = int(64*cfg.add_filter_ratio)
    else:
      self.inplanes = 64 + cfg.add_filter_num
    super(ResNet, self).__init__()
    self.conv1 = nn.Conv2d(3, 64 + cfg.add_filter_num, kernel_size=7, stride=2, padding=3,
                 bias=False)                                        # rcnn.base.0
    self.bn1 = nn.BatchNorm2d(64 + cfg.add_filter_num)              # rcnn.base.1
    ...
    if cfg.add_filter_ratio >= 0.005: 
      self.fc = nn.Linear((512 + int(512*cfg.add_filter_ratio)) * block.expansion, num_classes)
    else:
      self.fc = nn.Linear((512 + cfg.add_filter_num) * block.expansion, num_classes)
  ...
  def _make_layer(self, block, planes, blocks, stride=1):
    ...
    downsample_fa = None
    if cfg.add_filter_ratio >= 0.005: 
      planes = planes + int(planes*cfg.add_filter_ratio)
    else:
      planes = planes + cfg.add_filter_num
...
class resnet_add_filter(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    if self.pretrained == True:
      ...
      #   print(k)
      #   print(resnet.state_dict()[k].shape)
      new_state_dict = {}
      ...
      for k, v in state_dict.items():
        ...
        if len(k_list) == 3:
            if 'RCNN_base' in k:
                ...
                if current == 'conv1.weight':
                    params = resnet.state_dict()[current]
                    add_num = self.add_filter_num(v.shape[0])
                    params[:params.shape[0]-add_num] = v
                    new_state_dict[current] = params
                elif current in resnet.state_dict():
                    new_state_dict[current] = resnet.state_dict()[current]
                    params = resnet.state_dict()[current]
                    if len(params.shape) > 3:
                      add_num1,add_num2 = self.add_filter_num2(v.shape[0], v.shape[1])
                      params[:params.shape[0]-add_num1, :params.shape[1]-add_num2] = v
                    else:
                      add_num = self.add_filter_num(v.shape[0])
                      params[:params.shape[0]-add_num] = v
                    new_state_dict[current] = params
                    #print(current)
                    #print('current model shape is: ',new_state_dict[current].shape)
        elif len(k_list) > 3:
                ...
                if 'downsample' in k:
                    ...
                    if cfg.fa_conv_num == 1:
                        ...
                        if cfg.finetuneBN_DS and current_fa_0 in resnet.state_dict():
                            params = resnet.state_dict()[current_fa_0]
                            if len(params.shape) > 3:
                              add_num1,add_num2 = self.add_filter_num2(v.shape[0], v.shape[1])
                              params[:params.shape[0]-add_num1, :params.shape[1]-add_num2] = v
                            else:
                              add_num = self.add_filter_num(v.shape[0])
                              params[:params.shape[0]-add_num] = v
                            new_state_dict[current_fa_0] = params
                            #print(current)
                            #print('current model shape is: ',new_state_dict[current].shape)        
                    ...
                    if cfg.fa_conv_num == 2:
                        ...
                        if cfg.finetuneBN_DS and current_fa_0 in resnet.state_dict():
                            ...
                            #print(current_fa_0)
                            #print('current model shape is: ',new_state_dict[current_fa_0].shape)
                        if cfg.finetuneBN_DS and current_fa_2 in resnet.state_dict():
                            if 'downsample.0.' in k: # if it is .0. in k, weight belong to conv layer
                                params = resnet.state_dict()[current_fa_2]
                                if len(params.shape) > 3:
                                  add_num1,add_num2 = self.add_filter_num2(v.shape[0], v.shape[1])
                                  params[:params.shape[0]-add_num1, :params.shape[1]-add_num2] = v
                                else:
                                  add_num = self.add_filter_num(v.shape[0])
                                  params[:params.shape[0]-add_num] = v
                                new_state_dict[current_fa_2] = params
                                #print(current_fa_2)
                                #print('current model shape is: ',new_state_dict[current_fa_2].shape)
                        if cfg.finetuneBN_DS and current_fa_3 in resnet.state_dict():
                            if 'downsample.1.' in k: # if it is .1. in k, weight belong to bn layer
                                params = resnet.state_dict()[current_fa_3]
                                if len(params.shape) > 3:
                                  add_num1,add_num2 = self.add_filter_num2(v.shape[0], v.shape[1])
                                  params[:params.shape[0]-add_num1, :params.shape[1]-add_num2] = v
                                else:
                                  add_num = self.add_filter_num(v.shape[0])
                                  params[:params.shape[0]-add_num] = v
                                new_state_dict[current_fa_3] = params
                                #print(current_fa_3)
                                #print('current model shape is: ',new_state_dict[current_fa_3].shape)
    ...
    add_num1,add_num2 = self.add_filter_num2(512, 2048)
    if self.num_layers == 18:
      self.RCNN_cls_score_layers = nn.ModuleList([nn.Linear(512 + add_num1, n_classes) for n_classes in cfg.num_classes])
    else:
      self.RCNN_cls_score_layers = nn.ModuleList([nn.Linear(2048 + add_num2, n_classes) for n_classes in cfg.num_classes])

    if self.pretrained and cfg.Only_FinetuneBN:
      params_name_w = 'RCNN_cls_score.weight'
      params_name_b = 'RCNN_cls_score.bias'
      shape = state_dict[params_name_w].shape
      add_num1,add_num2 = self.add_filter_num2(shape[0], shape[1])
      shape = self.RCNN_cls_score_layers[1].weight.data.shape
      self.RCNN_cls_score_layers[1].weight.data[:,:shape[1]-add_num2] = state_dict[params_name_w]
      self.RCNN_cls_score_layers[1].bias.data = state_dict[params_name_b]

    add_num1,add_num2 = self.add_filter_num2(512, 2048)
    if self.class_agnostic:
      if self.num_layers == 18:
        self.RCNN_bbox_pred_layers = nn.ModuleList([nn.Linear(512 + add_num1,  4) for n_classes in cfg.num_classes])
      else:
        self.RCNN_bbox_pred_layers = nn.ModuleList([nn.Linear(2048 + add_num2, 4) for n_classes in cfg.num_classes])
    else:
      if self.num_layers == 18:
        self.RCNN_bbox_pred_layers = nn.ModuleList([nn.Linear(512 + add_num1,  4 * n_classes) for n_classes in cfg.num_classes])
      else:
        self.RCNN_bbox_pred_layers = nn.ModuleList([nn.Linear(2048 + add_num2, 4 * n_classes) for n_classes in cfg.num_classes])
    ...
    if self.pretrained and cfg.Only_FinetuneBN:
      ...
      add_num1,add_num2 = self.add_filter_num2(shape[0], shape[1])
      shape = self.RCNN_cls_score_layers[1].weight.data.shape
      self.RCNN_bbox_pred_layers[1].weight.data[:,:shape[1]-add_num2] = state_dict[params_name_w]
