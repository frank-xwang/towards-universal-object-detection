...
class resnet(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    if self.class_agnostic:
      if self.num_layers == 18:
        self.RCNN_bbox_pred_layers = nn.ModuleList([nn.Linear(512,  4) for batchsize in self.rpn_batchsize_list])
      else:
        self.RCNN_bbox_pred_layers = nn.ModuleList([nn.Linear(2048, 4) for batchsize in self.rpn_batchsize_list])
    else:
      if self.num_layers == 18:
