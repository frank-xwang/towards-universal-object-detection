...
def se_resnet18(pretrained=False):
  ...
  """
  model = SEResNet(SEBasicBlock, [2, 2, 2, 2])
  if pretrained:
    model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
  return model

def se_resnet34(pretrained=False):
  """Constructs a ResNet-34 model.
  Args:
    pretrained (bool): If True, returns a model pre-trained on ImageNet
  """
  model = SEResNet(SEBasicBlock, [3, 4, 6, 3])
  if pretrained:
...
def se_resnet50(pretrained=False):
  ...
  Args:
    pretrained (bool): If True, returns a model pre-trained on ImageNet
  """
  model = SEResNet(SEBottleneck, [3, 4, 6, 3])
#   if pretrained:
...
def se_resnet101(pretrained=False):
  ...
  Args:
    pretrained (bool): If True, returns a model pre-trained on ImageNet
  """
  model = SEResNet(SEBottleneck, [3, 4, 23, 3])
  if pretrained:
...
def se_resnet152(pretrained=False):
  ...
  Args:
    pretrained (bool): If True, returns a model pre-trained on ImageNet
  """
  model = SEResNet(SEBottleneck, [3, 8, 36, 3])
  if pretrained:
    model.load_state_dict(model_zoo.load_url(model_urls['resnet152']))
  return model

def load_pretrained(model, pretrained):
    """load pretrained model and initial non-existing parameters
    Args:
        pretrained: Pretrained model's state dict
        model: current model for loading pretrained parameters
    """
    ...
    for k, v in state_dict.items():
        ...
        if name in pretrained:
            loaded_params[k] = pretrained[name]
            state_dict[k] = pretrained[name]
            #print(' ',name, pretrained[name].shape)
            assert state_dict[k].cuda().sum() == pretrained[name].sum()
...
>>>>>>> 793eeda709a4483589939795954491531204c768
  ...
  def _init_modules(self):
    resnet = eval('se_resnet' + str(self.num_layers))()
    ...
    if self.pretrained == True:
      ...
      load_pretrained(resnet, state_dict)
