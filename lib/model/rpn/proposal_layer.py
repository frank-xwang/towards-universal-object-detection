...
from .bbox_transform import bbox_transform_inv, clip_boxes, clip_boxes_batch, bbox_overlaps_batch, bbox_transform_batch
...
class _ProposalLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        scores = input[0][:, self._num_anchors:, :, :] # fg probability
        bbox_deltas = input[1]
        im_info = input[2]
        cfg_key = input[3]
        if cfg.has_people:
        ...
        #print('TRAIN.RPN_NMS_THRESH',nms_thresh)
        ...
        #score_rois = scores.new(batch_size, post_nms_topN, 1).zero_()
        for i in range(batch_size):
            ...
            #score_rois[i,:num_proposal,1:] = scores_single
