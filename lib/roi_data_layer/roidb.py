...
def prepare_roidb(imdb):
  ...
  for i in range(len(imdb.image_index)):
    ...
    #if len(np.where(max_overlaps == 1)[0]) == 0:
    #  print('zero')
    #print('non zero number is: ', len(nonzero_inds))

    #assert all(max_classes[nonzero_inds] != 0)

def rank_roidb_ratio(roidb,imdb_names):
    # rank roidb based on the ratio between width and height.
    if 'kitti' in imdb_names or 'caltech' in imdb_names:
      print('current imdb name is kitti or caltech, ratio_large is set as 4(original is 2)')
      ratio_large = 4.0
      ratio_small = 0.5
    else:
...
def filter_roidb(roidb):
    ...
    while i < len(roidb):
      ...
      if min_x1 == max_x2 or min_y1 == max_y2:
        #print('error, bbox coordinate own the same value\n %s will be deleted'%(roidb[i]['image']))
        del roidb[i]
        i -= 1
        continue
      i+=1
...
def combined_roidb(imdb_names, training=True):
  ...
  #print('ratio index is: ' ,ratio_list)
